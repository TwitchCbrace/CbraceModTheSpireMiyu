...
public class UnexpectedAnglePower extends AbstractPower implements CloneablePowerInterface {
    ...
    public UnexpectedAnglePower(final AbstractCreature owner, final AbstractCreature source, final int amount, int damage) {
        name = NAME;
        ID = POWER_ID;
        this.owner = owner;
        this.amount = 5;
    ...
    public void onUseCard(AbstractCard card, UseCardAction action) {
        ...
        if (this.amount == 0) {
            ...
            this.addToBot(new ApplyPowerAction(AbstractDungeon.player,
                    AbstractDungeon.player, new VigorPower(AbstractDungeon.player, damage), damage));
        }
