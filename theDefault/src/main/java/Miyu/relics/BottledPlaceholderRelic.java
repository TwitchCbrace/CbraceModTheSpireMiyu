...
import Miyu.patches.relics.BottledPlaceholderField;
...
public class BottledPlaceholderRelic
    ...
    // This file will show you how to use 2 things - (Mostly) The Custom Bottle Relic and the Custom Savable - they go
    // hand in hand.
    ...
    // BasemodWiki Says: "When you need to store a value on a card or relic between runs that isn't a relic's counter
    // value
    // or a card's misc value, you use a custom savable to save and load it between runs."

    private static AbstractCard card; // The field value we wish to save in this case is the card that's going to be in
                                      // our bottle.
    ...
    public void onEquip() { // 1. When we acquire the relic
        ...
        AbstractDungeon.gridSelectScreen.open(group, 1, DESCRIPTIONS[3] + name + DESCRIPTIONS[2], false, false, false,
            false);
        // 6. Open the grid selection screen with the cards from the CardGroup we specified above. The description reads
        // "Select a card to bottle for" + (relic name) + "."
    }

    @Override
    public void onUnequip() { // 1. On unequip
        if (card != null) { // If the bottled card exists (prevents the game from crashing if we removed the bottled
                            // card from our deck for example.)
        ...
        for (Iterator<AbstractCard> it = AbstractDungeon.player.drawPile.group.iterator(); it.hasNext();) {
            ...
            if (BottledPlaceholderField.inBottledPlaceholderField.get(card)) { // Check if our SpireField matches said
                ...
                if (AbstractDungeon.player.hand.size() < BaseMod.MAX_HAND_SIZE) { // If your hand isn't full
                    ...
                    AbstractDungeon.player.drawPile.moveToHand(card, AbstractDungeon.player.drawPile); // RunAndGun the
                                                                                                       // card to your
                                                                                                       // hand from
                                                                                                       // your draw
                                                                                                       // pile
                ...
                else { // If your hand IS full - create a single "My hand is full!" dialogue and move the card to the
