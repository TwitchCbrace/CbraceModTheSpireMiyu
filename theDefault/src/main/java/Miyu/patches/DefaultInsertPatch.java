...
/*
 * Now let's imagine for a second that there were two methods named returnRandomRelicKey() The one we're patching -
 * "String returnRandomRelicKey(RelicTier tier)" - that grabs a relic of specific tier and a fictional one -
 * "String returnRandomRelicKey(RelicTier tier, LandingSound sound)" - that grabs a relic of a specific tier AND with a
 * specific landing sound. How would we tell the code which of the two methods to put our patch in? We use paramtypez
 * (read the docs too they have a good example!) Let's say we wanted to patch the second fictional one - we would add
 * paramtypez={ AbstractRelic.RelicTier.class, AbstractRelic.LandingSound.class } to this annotation, after the method
 * parameter. (If we wanted to patch the first one, we'd only put "AbstractRelic.RelicTier.class".
 */
)
public class DefaultInsertPatch {// Don't worry about the "never used" warning - *You* usually don't use/call them
                                 // anywhere. Mod The Spire does.

    // You can have as many inner classes with patches as you want inside this one - you don't have to separate each
    // patch into it's own file.
    // So if you need to put 4 patches all for 1 purpose (for example they all make a specific relic effect happen) -
    // you can keep them organized together.
    ...
    @SpireInsertPatch( // This annotation of our patch method specifies the type of patch we will be using. In our case
                       // - a Spire Insert Patch

        locator = Locator.class, // Spire insert patches require a locator - this isn't something you import - this
                                 // is something we write.
        // (Or as is usually the case with them - copy paste cause they're always nearly the same thing.
        // In fact, most insert patches are fairly boiler-plate. You could easily make an insert patch template, if
        // you'd like.)
        // You can find our Locator class just below, as an inner class, underneath our actual patch method.

        localvars = {"retVal"} // The method we're patching, returnRandomRelicKey(), has a local variable that we'd
                               // like to access and manipulate -
    // "String retVal = null;". So, we simply write out it's name here and then add it as a parameter to our patch
    // method.
    // Keep in mind that localvars can also be used to capture class variables, not just local method ones. This also
    ...
    public static void thisIsOurActualPatchMethod(
        ...
        // 3. localvars are passed as arguments, appearing the in parameter list after the original method's
        // parameters."

        // Example: if returnEndRandomRelicKey(RelicTier tier) were NOT static we would write our method parameters
        // as such:
        // thisIsOurActualPatchMethod(AbstractDungeon __instance, AbstractRelic.RelicTier tier, String retVal)
        // As it stands, that method is static so it's not tied to a specific instance of AbstractDungeon. (Read up
        // on what "static" means in java
        // if you don't understand this part).
        // As such we write our method parameters like this instead:
        AbstractRelic.RelicTier tier,
        String retVal) {

        // Wow time to actually put stuff in the basegame code!!! Everything here will be executed exactly as written,
        // at the line which we specified.
        // You can change retVal (using @byRef) to always return the same relic, or return a specific relic if it passes
        // some check.
        // You can execute any other static method you have, you can save retVal to your personal public static variable
        ...
        // The reason behind that is because by default, in patches, all variables are passed by Value, not by
        // Reference.
        // (You can google what this means in java if you don't understand).
        // This means that while we can change retVal within our own method, it won't change in the actual, original
        // basegame method.
        // If you want to do that, you'll need to use @byRef on the variable you want to change - this makes it get
        // passed by reference.
        // In our case that would be retVal - so we can annotate it with @byRef in our parameters. Another thing to note
        // is that non-array
        ...
        // Then, when we want to use it, can just access the (for us) one and only value in that array of Strings -
        // which would be placed at index 0.
        // retVal[0] = Anchor().relicID
        // Then the retVal would actually be changed outside of this method - inside returnRandomRelicKey();
    }

    private static class Locator
        extends SpireInsertLocator { // Hey welcome to our SpireInsertLocator class!
        @Override
        public int[] Locate(CtBehavior ctMethodToPatch) throws Exception {// All the locator has and needs is an
            ...
            // The Locate method is of type int[] - it returns an array of ints. These ints are actually the matching
            // line numbers.

            // This is where we open up the https://github.com/kiooeht/ModTheSpire/wiki/Matcher documentation.

            // The line in the original method, "return !RelicLibrary.getRelic(retVal).canSpawn() ?
            // returnEndRandomRelicKey(tier) : retVal;"
            ...
            // We want to insert our code immediately above it so we'll need to use a matcher against something in that
            // line.
            // We have a few of options for this 1 particular line. Before you proceed, read the docs and see how many
            // you can personally spot.
            ...
            // (get relic() returns an AbstractRelic so we just use canSpawn() directly from it to check if we can spawn
            // it)
            // 3. returnEndRandomRelicKey(tier) - is calling the returnEndRandomRelicKey method of the AbstractDungeon
            // class.
            // At the end of the day, all three of these are MethodCallMatchers.

            /*
             * BUT WAIT! Did you know that the third option is *actually* on a completely different line than the first
             * one? Decompiling the code with a different decompiler shows that the last line is actually a lot more
             * like this:
             *
             * if (!RelicLibrary.getRelic(retVal).canSpawn()) { return returnEndRandomRelicKey(tier); } return retVal;
             *
             * Which means that if we use the third matcher we will insert code inside the if statement, while if we use
             * 1. or 2. - just outside of it.
             *
             * Follow this guide to learn how to decompile your game.
             * https://github.com/daviscook477/BaseMod/wiki/Decompiling-Your-Game Essentially - you would want to use
             * JD-GUI and Luyten *both* to get a precise look. (You can 100% still totally use the IntelliJ for
             * quick-referencing code, it is still very fast and convenient)
             *
             * On a sidenote, you should enable debug lines in intelliJ both for bugfixing and seeing what thing is
             * *really* on what line To do so: 1. Ctrl+Shift+A 2. Registry 3. Scroll down and set
             * decompiler.dump.original.lines to true
             */

            // A good way to choose, usually, would be to pick the matcher least likely to appear elsewhere in the code
            // of the method
            // i.e. the rarest one. In this case, it doesn't really matter as it's 3 of the same matcher, and none of
            // their methods
            ...
            // In our case "RelicLibrary.getRelic()" is called only once, in that particular return statement, so we can
            // just return it.
            ...
            // The first (and for us, only) instance of "RelicLibrary.getRelic()" would be at index 0. The second at
            // index 1, and so on.

            // Finally, if we wanted to insert our code before *every* line with a match, we would just skip the index
            // and return the whole list of lines:
