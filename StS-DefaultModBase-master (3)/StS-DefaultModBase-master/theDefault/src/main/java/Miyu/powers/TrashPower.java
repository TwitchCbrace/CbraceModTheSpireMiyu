...
public class TrashPower extends AbstractPower implements CloneablePowerInterface {
    ...
    public void reducePower (int reduceAmount) {
        if (this.amount - reduceAmount <= 0) { this.fontScale = 8.0F;
            reduceAmount -= this.amount; this.amount = 0;
        } else {
            this.fontScale = 8.0F; this.amount -= reduceAmount;
        } for ( AbstractPower p : owner.powers )
        { if ( p instanceof OnReduceTrash) { ((OnReduceTrash) p).trashReduced(reduceAmount); } } }

    @Override
    public void onRemove() { for ( AbstractPower p : owner.powers )
    { if ( p instanceof OnReduceTrash ) { ((OnReduceTrash) p).trashReduced(amount); } } }
